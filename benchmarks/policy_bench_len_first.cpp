...
// Test with LEN_FIRST policy
#ifndef ENCHANTUM_ENUM_DISPATCH_POLICY
#define ENCHANTUM_ENUM_DISPATCH_POLICY ENCHANTUM_ENUM_POLICY_LEN_FIRST
#endif
...
void run_benchmarks() {
    ...
    policy_name = "LEN_FIRST";
#else 
    policy_name = "AUTO";
#endif

    std::cout << "Running benchmarks for " << policy_name << " policy..." << std::endl;
    
    for (const auto& workload : workloads) {
        std::cout << "  Workload: " << workload << std::endl;
        
        results.push_back(benchmark_enum<Tiny3>(policy_name, workload));
        results.push_back(benchmark_enum<Small8>(policy_name, workload));
        results.push_back(benchmark_enum<Medium12>(policy_name, workload));
        results.push_back(benchmark_enum<Large16>(policy_name, workload));
        results.push_back(benchmark_enum<Large32>(policy_name, workload));
        results.push_back(benchmark_enum<Large64>(policy_name, workload));
        results.push_back(benchmark_enum<BucketHeavy16>(policy_name, workload));
    }
    
    // Output results as CSV for easy parsing
    std::cout << "\n=== CSV RESULTS ===\n";
    std::cout << "enum,policy,workload,mean_ns,p90_ns\n";
    for (const auto& result : results) {
        std::cout << result.enum_name << "," 
                  << result.policy << ","
                  << result.workload << ","
                  << std::fixed << std::setprecision(2) << result.mean_ns << ","
                  << std::fixed << std::setprecision(2) << result.p90_ns << "\n";
    }
}
